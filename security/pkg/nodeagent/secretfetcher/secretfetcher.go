...
// SecretFetcher通过监听k8s secrets或者发送CSR到CA来获取secret
type SecretFetcher struct {
	// If CaClient is set, use caClient to send CSR to CA.
	// 如果CaClient被设置，使用caClient来发送CSR到CA
	...
	// FallbackSecretName存储fallback secret的名字，它在环境变量INGRESS_GATEWAY_FALLBACK_SECRET中被设置
	// 如果INGRESS_GATEWAY_FALLBACK_SECRET为空，则使用gateway-fallback作为fallback secret的默认名字
	// 如果fallback secret存在，FindGatewaySecret返回这个fallback secret，如果期望的secret不可用
...
// "token" field.
// isGatewaySecret检查secret并且决定是否这是一个用于Ingress gateway的secret
// 对于有着前缀"istio"以及"prometheus"的secrets，他们由istio system生成并且他们不是gateway secrets
// 其他istio由istio生成的secrets有着"token"字段，isGatewaySecret返回false，如果secret有着前缀"istio"或者
// "prometheus"，或者有着"token"字段
...
// extractK8sSecretIntoSecretItem从k8s Secret中抽取出一个server的cert/key对以及一个client的CA certificate
// 并且写入一对SecretItems，返回SecretItems以及一个布尔值表示这是否是一个只有CA的k8s Secret
// A CA only k8s secret has name suffix `-cacert`, and is ONLY considered for a client CA;
// 一个只有CA的k8s secret有后缀名为'-cacert'并且只被认为是一个client CA
// either a `cacert` or `tls.crt` must be provided.
// 一个'cacert'或者'tls.crt'必须被提供
...
// FindGatewaySecret返回名字和key匹配的secret，或者empty key如果没有secret存在，ok表明secret是否找到
// If there is a fallback secret named FallbackSecretName, return the fall back secret.
// 如果有一个叫FallbackSecretName的fallback secret存在，返回fall back secret
func (sf *SecretFetcher) FindGatewaySecret(key string) (secret security.SecretItem, ok bool) {
	...
	if !exist {
		...
		// 期望的secret不存在，试着找到fallback secret
