...
func TestEds(t *testing.T) {
	...
	// 使能本地的负载均衡并且增加相关的endpoints，为了进行测试
...
// address.
// 确认server发送了endpoint，这用给定的地址检查单个的endpoint
...
func edsUpdateInc(s *xds.FakeDiscoveryServer, adsc *adsc.ADSC, t *testing.T) {
	...
	// Will trigger a push.
	// 和K8S监听Service产生的事件相同，会触发一个push
	s.Discovery.MemRegistry.SetEndpoints(edsIncSvc, "",
...
// 创建一个直接的EDS grpc请求到pilot，确认结果是符合预期的
...
func multipleRequest(s *xds.FakeDiscoveryServer, inc bool, nclients,
	...
	// be detected
	// Bad client - 不会读取任何response，这会触发写阻塞，这应该被检测到
	...
	for i := 0; i < n; i++ {
		...
		go func(id int) {
			...
			// Pushes可能会被合并，这样我们就不会获得nPushes个推送
